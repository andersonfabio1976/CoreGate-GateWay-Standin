...
@Component
public class FinalizerMapperImpl implements FinalizerMapper {

    @Override
    public FinalizerResponseProto toProto(FinalizerResponseDto dto) {
        if ( dto == null ) {
            return null;
        }

        FinalizerResponseProto.Builder finalizerResponseProto = FinalizerResponseProto.newBuilder();

        finalizerResponseProto.setAuthorizationResult( authorizationResultToAuthorizationResult( dto.authorizationResult() ) );

        return finalizerResponseProto.build();
    }

    @Override
    public FinalizerRequestProto toProto(FinalizerRequestDto dto) {
        if ( dto == null ) {
            return null;
        }

        FinalizerRequestProto.Builder finalizerRequestProto = FinalizerRequestProto.newBuilder();

        return finalizerRequestProto.build();
    }

    @Override
    public FinalizerResponseDto toDto(FinalizerResponseProto proto) {
        if ( proto == null ) {
            return null;
        }

        FinalizerResponseDto.FinalizerResponseDtoBuilder finalizerResponseDto = FinalizerResponseDto.builder();

        if ( proto.hasAuthorizationResult() ) {
            finalizerResponseDto.authorizationResult( authorizationResultToAuthorizationResult1( proto.getAuthorizationResult() ) );
        }

        return finalizerResponseDto.build();
    }

    @Override
    public FinalizerRequestDto toDto(FinalizerRequestProto proto) {
        if ( proto == null ) {
            return null;
        }

        FinalizerRequestDto.FinalizerRequestDtoBuilder finalizerRequestDto = FinalizerRequestDto.builder();

        return finalizerRequestDto.build();
    }
    ...
    protected AuthorizationResult authorizationResultToAuthorizationResult(br.com.coregate.application.dto.transaction.AuthorizationResult authorizationResult) {
    ...
    protected br.com.coregate.application.dto.transaction.AuthorizationResult authorizationResultToAuthorizationResult1(AuthorizationResult authorizationResult) {
        if ( authorizationResult == null ) {
            return null;
        }

        br.com.coregate.application.dto.transaction.AuthorizationResult.AuthorizationResultBuilder authorizationResult1 = br.com.coregate.application.dto.transaction.AuthorizationResult.builder();
