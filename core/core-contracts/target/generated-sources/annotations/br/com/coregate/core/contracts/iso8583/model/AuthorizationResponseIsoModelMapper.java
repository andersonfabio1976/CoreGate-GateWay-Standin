...
public class AuthorizationResponseIsoModelMapper {

    public static AuthorizationResponseIsoModel decode(String iso) {
        AuthorizationResponseIsoModel model = new AuthorizationResponseIsoModel();
        int offset = 0;
        ...
        for (int i = 1; i <= 64; i++) {
            ...
            if (i == 38) {
                String val = iso.substring(offset, offset + 6).trim();
                offset += 6;
                model.setAuthIdResponse(val);
            }
            if (i == 39) {
                String val = iso.substring(offset, offset + 2).trim();
                offset += 2;
                model.setResponseCode(val);
            }
            if (i == 41) {
    ...
    public static String encode(AuthorizationResponseIsoModel model) {
        ...
        {
            sb.append(padRight(model.getAuthIdResponse(), 6));
        }
        {
            sb.append(padRight(model.getResponseCode(), 2));
        }
        {
